...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(n_layers):
            name = "hidden_" + str(i)
            if i == 0:
                hidden_layer = tf.layers.dense(input_placeholder, size, activation=activation, name=name)
            else:
                hidden_layer = tf.layers.dense(hidden_layer, size, activation=activation, name=name)
        output_placeholder = tf.layers.dense(hidden_layer, output_size, activation=output_activation, name="output")
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            # Logits of a categorical distribution
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "policy_forward_pass", self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            # Mean and log standard deviation of a multivariate Gaussian
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            sy_logprob_n = tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(sy_ac_na, depth=self.ac_dim),
                                                                       logits=sy_logits_na)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            for reward_path in re_n:
                ...
                # Calculating reward for each timestep
                for i, reward in enumerate(reward_path):
                    reward_to_go_i = len(reward_path) - i
                    gamma = np.power(self.gamma, np.arange(reward_to_go_i))
                    r_to_go = reward_path[i:]
                    r_sum = np.sum(r_to_go * gamma)
                    path_e.append(r_sum)

                # append the path's array of estimated returns
                rewards.append(np.array(path_e))

        else:
            for reward_path in re_n:
                t_prev = np.arange(len(reward_path))
                gamma = np.power(self.gamma, t_prev)
                discounted_reward = reward_path * gamma
                r_sum = np.sum(discounted_reward)
                path_r = r_sum * np.ones_like(reward_path)
...
def main():
    ...
    for e in range(args.n_experiments):
        ...
        print("Hakkan tegema uusi protsesse")

        p = Process(target=train_func, args=tuple())

        print("protsessi tegemisega sain hakkama")
        p.start()
        print("käima sain ka ")
        processes.append(p)
