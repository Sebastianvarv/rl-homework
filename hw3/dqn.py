...
class QLearner(object):
    ...
    def step_env(self):
        ...
        enc_obs_batch = self.replay_buffer.encode_recent_observation()

        if self.model_initialized:

            q_values = self.session.run(self.q_t, feed_dict={self.obs_t_ph: np.expand_dims(enc_obs_batch, 0)})
        ...
        self.replay_buffer.store_effect(idx=frame_idx, action=action, reward=reward, done=done)
        self.last_obs = obs
